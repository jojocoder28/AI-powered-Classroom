    ...
    switch (formData.role) {
        case 'Student':
          return (
            ...
            <InputField label="Interests" name="interests" value={formData.interests} onChange={handleChange} />
      ...
      case 'Teacher':
        return (
          <>
            ...
            <InputField label="Department" name="department" value={formData.department} onChange={handleChange} required />
             <InputField label="About" name="about" value={formData.about} onChange={handleChange} type='text' />
      ...
      <div className="max-w-lg w-full space-y-8 bg-white dark:bg-gray-700 rounded-xl shadow-lg p-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
           ...
           <div className="mt-4">
            ...
            <button type="submit" disabled={loading} className={`w-full px-4 py-2 mt-4 text-sm font-medium text-white rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 ${loading ? 'opacity-50 cursor-not-allowed' : 'bg-gradient-to-tr from-teal-600 to-teal-800 hover:from-teal-700 hover:to-teal-900'}`}>
                {loading ? 'Registering...' : 'Register'}
            </button>
        </form>
        <div className="mt-6 text-center text-sm text-gray-600 dark:text-gray-400 flex flex-col items-center">
           <p>Already have an account?</p>
           <Link to="/login" className="font-medium text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300 w-full block mt-2">
               <button className='w-full border-2 border-teal-500 px-2 py-1 rounded-md hover:bg-teal-500 hover:text-gray-50 '>Sign in</button>
           </Link>
        </div>
      </div>
    </div>
